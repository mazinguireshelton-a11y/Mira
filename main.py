...
def gerar_proposta_ia(nome, nicho, site, avaliacao, objetivo, remetente_nome):
    ...
    Quem envia a mensagem se chama: {remetente_nome}.
    Retorna APENAS:
    1. DIAGNÓSTICO: (1 frase curta)
    2. MENSAGEM: (1 mensagem persuasiva de WhatsApp, assinada com "{remetente_nome}")
    """
    erros = []
...
def gerar_proposta(req: PropostaRequest, user=Depends(get_current_user)):
    ...
    texto, erro = gerar_proposta_ia(req.nome, req.nicho, req.site, req.avaliacao, req.objetivo, req.remetente_nome)
